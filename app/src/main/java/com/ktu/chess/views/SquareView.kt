...
import com.ktu.chess.chessPieces.Pawn
import com.ktu.chess.chessPieces.Queen
...
class SquareView(
    ...
    private fun onSquareClick() {
        ...
        if (boardColor != b.color) {
            ...
            if (selected != null) {
                data.setPiece(selected.content)
                ...
                if (data.content is Pawn) {
                    ...
                    if (data.y == 0 || data.y == data.board.length-1) {
                        data.setPiece(Queen(piece.team, data.board,
                            if (piece.team == "BLACK") {data.board.drawables.BLACK_QUEEN} else {data.board.drawables.WHITE_QUEEN}))
                    }
                }
            }
